...
function serializeRequest(vr: {
  ...
  createdAt: Date;
  updatedAt: Date;
}, includeVehicle = false): SerializedRequest {
  const vehicleIdRaw = vr.vehicleId as unknown as { _id: Types.ObjectId; plate: string; ownerName: string; vehicleType: string; status: string; userId?: Types.ObjectId; isCompanyVehicle: boolean; createdAt: Date; updatedAt: Date; brand?: string; chassisNo?: string; color?: string; engineNo?: string; model?: string; ownerAddress?: string; year?: number } | null;
  const vehicleIdStr = vehicleIdRaw != null && typeof vehicleIdRaw === "object" && "_id" in vehicleIdRaw
    ? vehicleIdRaw._id.toString()
    : String(vr.vehicleId);

  const subscriptionIdRaw = vr.subscriptionId as unknown as { _id: Types.ObjectId } | null;
  const subscriptionIdStr = subscriptionIdRaw != null && typeof subscriptionIdRaw === "object" && "_id" in subscriptionIdRaw
    ? subscriptionIdRaw._id.toString()
    : String(vr.subscriptionId);

  const userIdRaw = vr.userId as unknown as { _id: Types.ObjectId } | null;
  const userIdStr = userIdRaw != null && typeof userIdRaw === "object" && "_id" in userIdRaw
    ? userIdRaw._id.toString()
    : String(vr.userId);

  const resolvedByRaw = vr.resolvedBy as unknown as { _id: Types.ObjectId } | null;
  const resolvedByStr = resolvedByRaw != null && typeof resolvedByRaw === "object" && "_id" in resolvedByRaw
    ? resolvedByRaw._id.toString()
    : vr.resolvedBy?.toString();

  const vehicle = includeVehicle && vehicleIdRaw != null
  ...
  vehicle,
  user: (() => {
    const u = vr.userId as unknown as { _id: Types.ObjectId; name?: string; email?: string; phone?: string } | null;
...
export async function listVehicleRequests(request: Request, response: Response) {
...
export async function createVehicleRequest(request: Request, response: Response) {
  const body = z
    .object({
  ...
  if (sub.userId.toString() !== request.user?.id) {
    response.status(403).json({ message: "Bạn không có quyền gửi yêu cầu cho gói này." });
    return;
  }
  ...
  if (existing) {
    response.status(409).json({ message: "Bạn đã có yêu cầu đang chờ duyệt cho xe này." });
    return;
  }
  ...
  response.status(201).json({
    request: {
      ...
      vehicle: undefined,
      user: request.user ? { name: request.user.name, email: request.user.email ?? null, phone: null } : null,
...
export async function resolveVehicleRequest(request: Request, response: Response) {
  const body = z
    .object({
      id: z.string().min(1),
  ...
  vr.resolvedBy = request.user?.id ? new mongoose.Types.ObjectId(request.user.id) : undefined;
  vr.resolvedAt = new Date();

  if (body.action === "approved") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const vehicleDoc = (await Vehicle.findById(vr.vehicleId)) as any;
    const sub = vr.subscriptionId ? await Subscription.findById(vr.subscriptionId) : null;

    // Xe mới đăng ký → chỉ cần cập nhật status
    if (vr.type === "edit" && vr.requestedChanges?.status === "Đã đăng ký" && vehicleDoc) {
      ...
      if (changes.plate) {
        const existing = await Vehicle.findOne({ plate: (changes.plate as string).toUpperCase().replace(/[\s-]+/g, "") });
        ...
        vehicleDoc.plate = (changes.plate as string).toUpperCase().replace(/[\s-]+/g, "");
      }
      if (changes.ownerName !== undefined) vehicleDoc.ownerName = changes.ownerName as string;
      if (changes.ownerPhone !== undefined) vehicleDoc.ownerPhone = changes.ownerPhone as string | undefined;
      if (changes.ownerAddress !== undefined) vehicleDoc.ownerAddress = changes.ownerAddress as string | undefined;
      if (changes.brand !== undefined) vehicleDoc.brand = changes.brand as string | undefined;
      if (changes.model !== undefined) vehicleDoc.model = changes.model as string | undefined;
      if (changes.color !== undefined) vehicleDoc.color = changes.color as string | undefined;
      if (changes.year !== undefined) vehicleDoc.year = changes.year as number | undefined;
      if (changes.engineNo !== undefined) vehicleDoc.engineNo = changes.engineNo as string | undefined;
      if (changes.chassisNo !== undefined) vehicleDoc.chassisNo = changes.chassisNo as string | undefined;
      if (changes.imageUrl !== undefined) vehicleDoc.imageUrl = changes.imageUrl as string | undefined;
      await vehicleDoc.save();
    }
  ...
  await vr.populate({ path: "userId", model: "User", select: "name email phone" });

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const serialized = serializeRequest(vr as any, vr.type === "edit" && body.action === "approved");
  const extra = vr.type === "edit" && body.action === "approved"
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
