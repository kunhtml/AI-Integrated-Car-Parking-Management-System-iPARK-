...
export async function getRevenueReport(request: Request, response: Response) {
  ...
  const totalRevenue = sessions.reduce(
    (sum, session) =>
      sum + (["paid", "fully_paid"].includes(String(session.paymentStatus)) ? Number(session.fee || 0) : 0),
    0,
  );
  ...
  for (const session of sessions) {
    ...
    bucket.entries += 1;
    bucket.revenue += ["paid", "fully_paid"].includes(String(session.paymentStatus)) ? Number(session.fee || 0) : 0;
