...
async function resolveTarget(
  ...
  if (vehicleId && !vehicle)
    throw new AppError("Không tìm thấy xe để gán thẻ RFID.", 404);
  if (!vehicle)
    throw new AppError("Vui lòng chọn xe để bán thẻ RFID Member.", 400);
  const resolvedUserId = vehicle.userId;
  if (!resolvedUserId)
    throw new AppError("Xe chưa được gắn với tài khoản khách hàng.", 409);
  if (userId && resolvedUserId.toString() !== userId)
    throw new AppError("Tài khoản khách hàng không sở hữu xe đã chọn.", 409);
  const normalizedPlate = vehicle?.plate || plate?.trim().toUpperCase();
  const duplicateQuery: Record<string, unknown> = {
    cardType: "member",
    status: { $in: ["active", "in-use", "pending-sale"] },
  };
  if (vehicle)
    duplicateQuery.$or = [{ vehicleId: vehicle._id }, { plate: vehicle.plate }];
  else if (normalizedPlate) duplicateQuery.plate = normalizedPlate;
  if (normalizedPlate) {
    const duplicate = await RfidCard.findOne(duplicateQuery);
    if (duplicate)
      throw new AppError("Xe đã có thẻ RFID chính đang hoạt động.", 409);
  }
...
async function finalizeSale(transaction: TransactionDocument) {
  if (!transaction.rfidCardId)
    throw new AppError("Giao dịch không gắn với thẻ RFID.", 500);
  const card = await RfidCard.findOneAndUpdate(
    { _id: transaction.rfidCardId, status: "pending-sale" },
    {
      $set: {
        ...
        ownerName: transaction.vehicleId
          ? (await Vehicle.findById(transaction.vehicleId))?.ownerName ||
            "Thành viên"
          : "Thành viên",
        userType: "resident",
  ...
  if (!card) {
    ...
    if (
      ...
      ["active", "in-use"].includes(existing.status) &&
      existing.soldAt
    )
      return existing;
  ...
  if (!updatedTransaction)
    throw new AppError("Không thể cập nhật trạng thái giao dịch RFID.", 500);
  await writeAudit(
...
export async function sellRfidCard(input: SellRfidCardInput, actorId?: string) {
  if (input.salePrice < 0 || input.depositAmount < 0)
    throw new AppError("Giá bán và tiền cọc không được âm.", 400);
  if (
    input.salePrice + input.depositAmount > 0 &&
    input.method === "cash" &&
    !actorId
  )
    throw new AppError("Giao dịch tiền mặt phải có nhân viên thực hiện.", 403);
  if (input.salePrice + input.depositAmount === 0 && !input.freeReason?.trim())
    throw new AppError("Cấp thẻ miễn phí phải ghi rõ lý do.", 400);
  ...
  const card = await RfidCard.findOneAndUpdate(
    { _id: input.cardId, status: "available" },
    {
      $set: {
        status: "pending-sale",
        pendingTransactionId: new mongoose.Types.ObjectId(),
      },
    },
    { new: true },
  );
  if (!card)
    throw new AppError(
      "Thẻ không tồn tại hoặc không ở trạng thái available.",
      409,
    );
  ...
  try {
    const transaction = await Transaction.create({
      ...
      status:
        input.method === "payos" && input.salePrice + input.depositAmount > 0
          ? "pending"
          : "paid",
      paidAt:
        input.method === "payos" && input.salePrice + input.depositAmount > 0
    ...
    if (transaction.status === "paid") {
      ...
      if (!payosResult.success || !payosResult.orderCode) {
        await Transaction.findByIdAndUpdate(transaction._id, {
          $set: {
            status: "failed",
            note: payosResult.error || "Không tạo được link PayOS",
          },
        });
        await RfidCard.findByIdAndUpdate(card._id, {
          $set: { status: "available" },
          $unset: { pendingTransactionId: 1 },
        });
        throw new AppError(
          payosResult.error || "Không thể tạo thanh toán PayOS cho thẻ RFID.",
          502,
        );
      }
      ...
      return {
        ...
        card: await RfidCard.findById(card._id),
        payos: payosResult,
      };
    }
...
export async function sellRfidCardForCustomer(input: {
  userId: string;
  vehicleId: string;
  salePrice: number;
  depositAmount?: number;
  ...
  if (!card)
    throw new AppError(
      "Hiện không còn thẻ RFID để bán. Vui lòng thử lại sau.",
      409,
    );
  try {
...
export async function finalizePaidRfidTransaction(transactionId: string) {
  const transaction = await Transaction.findOne({
    _id: transactionId,
    transactionType: { $in: ["rfid_sale", "rfid_replacement"] } as any,
  });
  if (!transaction) throw new AppError("Không tìm thấy giao dịch RFID.", 404);
  if (transaction.status !== "paid")
    throw new AppError("Giao dịch RFID chưa thanh toán.", 409);
  const card = await finalizeSale(transaction);
  return { transaction, card };
}

export async function reconcileCustomerRfidSale(
  ...
  if (!transaction)
    throw new AppError("Không tìm thấy giao dịch mua thẻ RFID.", 404);
  ...
  return {
    ...
    card: transaction.rfidCardId
      ? await RfidCard.findById(transaction.rfidCardId)
      : null,
  };
}
...
export async function reconcilePendingRfidSales() {
  ...
  for (const transaction of pending) {
    const status = await checkPayOSPaymentStatus(
      String(transaction.payosOrderCode),
    );
...
export async function confirmRfidSale(transactionId: string, actorId?: string) {
  ...
  if (!transaction)
    throw new AppError("Không tìm thấy giao dịch bán thẻ RFID.", 404);
  if (transaction.status === "paid")
    return finalizePaidRfidTransaction(transactionId);
  if (transaction.status !== "pending")
    throw new AppError("Giao dịch không ở trạng thái chờ thanh toán.", 409);
...
export async function listRfidInventory(filters: {
  ...
  if (filters.search?.trim()) {
    ...
    query.$or = [
      { uid: new RegExp(value, "i") },
      { cardId: new RegExp(value, "i") },
      { plate: new RegExp(value, "i") },
      { ownerName: new RegExp(value, "i") },
    ];
  }
  ...
  const summary = await RfidCard.aggregate([
    { $group: { _id: "$status", count: { $sum: 1 } } },
  ]);
  return { items, total, page, limit, summary };
}

export async function listRfidTransactions(filters: {
  status?: string;
  page?: number;
  limit?: number;
}) {
  ...
  const query: Record<string, unknown> = {
    transactionType: {
      $in: ["rfid_sale", "rfid_replacement", "rfid_refund", "rfid_deposit"],
    },
  };
  if (filters.status) query.status = filters.status;
  const [items, total] = await Promise.all([
    Transaction.find(query)
      ...
      .skip((page - 1) * limit)
      .limit(limit)
      .populate("rfidCardId", "uid cardId cardType status")
      .populate("vehicleId", "plate ownerName"),
    Transaction.countDocuments(query),
  ]);
  return { items, total, page, limit };
}

export async function changeRfidCardStatus(
  id: string,
  status: Extract<RfidCardStatus, "lost" | "blocked" | "damaged" | "available">,
  reason: string | undefined,
  actorId?: string,
) {
  const card = await RfidCard.findById(id);
  if (!card) throw new AppError("Không tìm thấy thẻ RFID.", 404);
  if (status === "available" && card.status === "in-use")
    throw new AppError(
      "Không thể mở lại thẻ đang gán cho xe; hãy dùng quy trình trả thẻ.",
      409,
    );
  card.status = status;
  ...
  if (status === "lost") {
    ...
    // Gỡ liên kết với gói dịch vụ (Subscription.rfidCardId) khi báo mất.
  ...
  if (status === "blocked") {
    ...
    card.blockedReason = reason?.trim() || "Khóa theo yêu cầu vận hành";
  }
  if (status === "damaged") {
    card.damagedAt = new Date();
    card.damagedReason = reason?.trim() || "Thẻ hỏng";
    // Gỡ liên kết với gói dịch vụ (Subscription.rfidCardId) khi báo hỏng để
    // thành viên trở lại trạng thái chưa có RFID và có thể được cấp thẻ thay thế.
    await Subscription.updateMany(
      { rfidCardId: card._id },
      { $unset: { rfidCardId: 1 } },
    );
  ...
  await writeAudit(card.cardId ?? card.uid, auditAction, "success", actorId, {
    reason,
  });
  return card;
}

export async function returnRfidCard(
  id: string,
  options: {
  ...
  if (card.cardType !== "guest")
    throw new AppError(
      "RFID Member là thẻ bán đứt, không được trả về kho.",
      409,
    );
  if (!activeCardStatuses.includes(card.status))
    throw new AppError("Chỉ có thể trả thẻ Guest đang được sử dụng.", 409);
  ...
  if (options.refundDeposit && (card.depositAmount ?? 0) > 0) {
    refund = await Transaction.create({
      transactionType: "rfid_refund",
      rfidCardId: card._id,
      uid: card.uid,
      vehicleId: oldVehicleId,
      createdBy: objectId(actorId),
      method: "cash",
      amount: card.depositAmount ?? 0,
      depositAmount: card.depositAmount ?? 0,
      status: "paid",
      paidAt: new Date(),
      refundReason: options.refundReason || "Hoàn cọc khi trả thẻ",
    });
  }
  await writeAudit(card.cardId ?? card.uid, "return", "success", actorId, {
    inspectionPassed: options.inspectionPassed,
    refundDeposit: options.refundDeposit,
  });
  return { card, refund };
}
...
export async function replaceRfidCard(
  id: string,
  input: Omit<SellRfidCardInput, "replacementOf">,
  actorId?: string,
) {
  const oldCard = await RfidCard.findById(id);
  if (!oldCard) throw new AppError("Không tìm thấy thẻ RFID cũ.", 404);
  if (oldCard.cardType !== "member")
    throw new AppError("Chỉ cấp lại thẻ cho RFID Member bán đứt.", 409);
  if (!["lost", "damaged", "blocked"].includes(oldCard.status))
    throw new AppError(
      "Chỉ được cấp lại thẻ cho thẻ mất, hỏng hoặc bị khóa.",
      409,
    );
  const result = await sellRfidCard(
    {
      ...input,
      cardType: "member",
      userId: input.userId || oldCard.userId?.toString(),
      vehicleId: input.vehicleId || oldCard.vehicleId?.toString() || "",
      replacementOf: oldCard._id.toString(),
    },
    actorId,
  );
  ...
  await writeAudit(
    ...
    "success",
    actorId,
    { replacementCardId: result.card?._id?.toString() },
  );
