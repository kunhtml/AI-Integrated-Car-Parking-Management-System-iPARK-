...
async function loadAvailableInventoryCards() {
  const firstPage = await getRfidInventory({
    status: "available",
    limit: 100,
    page: 1,
  });
  ...
  return [firstPage, ...rest]
    ...
    .sort((firstCard, secondCard) =>
      firstCard.uid.localeCompare(secondCard.uid),
    );
}

interface RfidIssueManagerPanelProps {
  /** Được gọi ngay sau khi cấp thẻ thành công để cha refresh dữ liệu (bảng thẻ RFID). */
  onCardIssued?: () => void | Promise<void>;
}

export function RfidIssueManagerPanel({
  onCardIssued,
}: RfidIssueManagerPanelProps) {
  ...
  async function load() {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Không thể tải dữ liệu RFID.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function action(id: string, actionType: "approve" | "reject") {
    const response = await api.reviewRfidPurchaseRequest(id, {
      action: actionType,
      reason: note[id],
    });
    ...
    try {
      ...
      setMessage(
        response.ok
          ...
          : data.message || "Không thể cấp thẻ.",
      );
      if (response.ok) {
      ...
      <div className="panel-header">
        <div>
          ...
          <p className="muted-text">
            Chỉ xử lý yêu cầu đã thanh toán: duyệt, chọn thẻ còn trống trong kho
              ...
              {item.status === "waiting_issuance" && (
                <>
                  <textarea
                    ...
                    value={note[item.id] || ""}
                    onChange={(event) =>
                      setNote({ ...note, [item.id]: event.target.value })
                    }
              ...
              {item.status === "approved_waiting_assignment" && (
                ...
                >
                  <select
                    ...
                    disabled={
                      assigningId === item.id || availableCards.length === 0
                    }
                    style={{ minWidth: 260, flex: "1 1 260px" }}
                  >
                    <option value="">
                      {availableCards.length
                        ? "Chọn thẻ RFID còn trống trong kho"
                        : "Kho hiện không còn thẻ RFID trống"}
                    </option>
