...
export function RfidCardsView() {
  ...
  async function toggleStatus(card: RfidCardItem) {
    if (submitting) return;
    const nextStatus = card.status === "active" ? "inactive" : "active";
    setSubmitting(true);
    try {
      const res = await apiFetch(`/rfid/${card.id}/status`, {
        method: "POST",
            ...
            rows={filtered.map((card) => [
              ...
              <div key="actions" className="rfid-actions">
                ...
                <button
                  ...
                  disabled={submitting}
                  title={card.status === "active" ? "Khóa thẻ" : "Mở khóa thẻ"}
                  type="button"
                  style={{ color: card.status === "active" ? "#f59e0b" : "#16a34a" }}
                >
                  {card.status === "active" ? <XCircle size={13} /> : <CheckCircle2 size={13} />}
                </button>
                {isAdmin && (
                  <button
                    className="small-button"
