...
function CameraLaneTest({
  ...
  const [plateStatus, setPlateStatus] = useState(
    "Đưa xe vào khung hình để kiểm tra OCR.",
  );
  ...
  async function testRfid() {
    ...
    try {
      const start = await bridgeFetch("/api/rfid/scan/start", {
        method: "POST",
        body: JSON.stringify({ direction: lane, mode: "inventory" }),
      });
      ...
      while (Date.now() < deadline) {
        ...
        const response = await bridgeFetch(
          `/api/rfid/scan/poll?direction=${lane}`,
        );
        const data = await response.json().catch(() => ({}));
        if (data.status === "success" && data.uid) {
          setRfidStatus(`Đọc thành công UID: ${data.uid}`);
          return;
        }
      ...
      setRfidStatus(
        error instanceof Error ? error.message : "Lỗi kiểm tra RFID.",
      );
    } finally {
      setTesting(false);
      void bridgeFetch("/api/rfid/scan/cancel", {
        method: "POST",
        body: JSON.stringify({ direction: lane }),
      });
    }
  }
  ...
  async function testGate() {
    ...
    try {
      ...
      setGateStatus(
        error instanceof Error ? error.message : "Lỗi kiểm tra barie.",
      );
    }
  }
  ...
  useEffect(() => {
    const interval = window.setInterval(async () => {
      ...
      const log = Array.isArray(data.logs)
        ? data.logs.find(
            (item: {
              direction?: string;
              ...
              item.detectedPlate &&
              item.createdAt &&
              new Date(item.createdAt).getTime() >= startedAt,
          )
        : null;
      if (log?.detectedPlate) {
        setPlateStatus(`Nhận diện gần nhất: ${log.detectedPlate}`);
        const imagePath =
          typeof log.imagePath === "string" ? log.imagePath : "";
        setPlateCropUrl(
          imagePath
            ? imagePath.startsWith("http")
              ? imagePath
              : `${bridgeBaseUrl}${imagePath.startsWith("/") ? "" : "/"}${imagePath}`
            : "",
        );
      }
    }, 1500);
    ...
    >
      <div className="modal devices-test-modal">
        <div className="modal-header">
          <div>
            <p className="muted-text">Kiểm tra lane {lane.toUpperCase()}</p>
            <h3>Test {device.name}</h3>
          </div>
          <button
            type="button"
        ...
        <div className="device-test-stream">
          <img src={streamUrl} alt={`Camera ${device.name}`} />
        </div>
        <div className="device-test-results">
          <p>
            <strong>Camera / OCR:</strong> {plateStatus}
          </p>
          <p>
            <strong>RFID:</strong> {rfidStatus}
          </p>
        </div>
        <div className="device-test-crop">
          <strong>Ảnh crop biển số</strong>
          {plateCropUrl ? (
            <img src={plateCropUrl} alt="Ảnh crop biển số nhận diện" />
          ) : (
            <span>Chưa nhận được ảnh crop từ lần OCR gần nhất.</span>
          )}
        </div>
        <div className="device-test-actions">
          <button
            ...
            disabled={testing}
            onClick={() => void testRfid()}
          >
            <Nfc size={14} />{" "}
            {testing ? "Đang quét RFID..." : "Test đầu đọc RFID"}
          </button>
          <button
            type="button"
...
export function DevicesView() {
  ...
  const loadDevices = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await responseMessage(response, "Không tải được danh sách thiết bị."),
        );
      }
      const data = await response.json();
      const nextDevices = Array.isArray(data.devices)
        ? (data.devices as DeviceItem[])
        : [];
  ...
  async function swapCameraRoles() {
    ...
    setError("");
    try {
      const response = await apiFetch("/devices/swap-roles", {
        method: "POST",
      });
      const message = await responseMessage(
        response,
        "Không thể hoán đổi vai trò camera.",
      );
      if (!response.ok) {
        setError(message);
        return;
  ...
  async function runDeviceAction(
    ...
    setNotice("");
    try {
      const response = await apiFetch(`/devices/${device.id}/${action}`, {
        method: "POST",
      });
      const message = await responseMessage(
        response,
        action === "restart"
          ? "Không khởi động lại được thiết bị."
          : "Không chụp được snapshot.",
      );
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Thao tác với thiết bị thất bại.",
      );
    } finally {
      setActionId(null);
    }
  ...
  async function deleteDevice(device: DeviceItem) {
    ...
    setError("");
    try {
      const response = await apiFetch(`/devices/${device.id}`, {
        method: "DELETE",
      });
      const message = await responseMessage(
        response,
      ...
      setError(
        cause instanceof Error ? cause.message : "Không thể xóa thiết bị.",
      );
    } finally {
      setActionId(null);
    }
  ...
  async function createDevice(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await apiFetch(
        editingDevice ? `/devices/${editingDevice.id}` : "/devices",
        {
      ...
      const message = await responseMessage(
        response,
        editingDevice
          ? "Không cập nhật được thiết bị."
          : "Không tạo được thiết bị.",
      );
      if (!response.ok) throw new Error(message);
      setNotice(
        editingDevice
          ? "Đã cập nhật thiết bị camera."
          : "Đã thêm thiết bị camera.",
      );
      setForm(EMPTY_FORM);
      ...
      setError(
        cause instanceof Error ? cause.message : "Không lưu được thiết bị.",
      );
    } finally {
      setSaving(false);
    }
  ...
  function updateForm<K extends keyof DeviceForm>(
    key: K,
    value: DeviceForm[K],
  ) {
        ...
        <div className="devices-toolbar">
          ...
          <label className="select-field">
            ...
            <select
              value={gateFilter}
              onChange={(event) =>
                setGateFilter(event.target.value as GateFilter)
              }
            >
          ...
          >
            ...
            {formOpen ? "Đóng form" : "Thêm thiết bị"}
          </button>
          ...
          <form className="device-create-form" onSubmit={createDevice}>
            ...
            <div className="device-form-grid">
              ...
              <label>
                ...
                <select
                  value={form.gate}
                  onChange={(event) =>
                    updateForm("gate", event.target.value as DeviceForm["gate"])
                  }
                >
                  <option value="entry">Cổng vào</option>
                  <option value="exit">Cổng ra</option>
              ...
              <label className="device-form-wide">
                ...
                <input
                  ...
                  value={form.rtspUrl}
                  onChange={(event) =>
                    updateForm("rtspUrl", event.target.value)
                  }
              ...
              <label className="device-form-wide">
                ...
                <input
                  value={form.roiNote}
                  onChange={(event) =>
                    updateForm("roiNote", event.target.value)
                  }
                  placeholder="Ví dụ: ROI khu vực biển số"
                />
              </label>
            </div>
            <div className="device-form-actions">
              ...
              >
                ...
                {editingDevice ? "Lưu thay đổi" : "Tạo thiết bị"}
              </button>
            </div>
          </form>
        ) : null}
                ...
                <article key={device.id} className="device-card">
                  <div className="device-card-heading">
                    <div className="device-card-title">
                      ...
                      <div>
                        ...
                        <span>{gateLabel(device.gate)}</span>
                      </div>
                    </div>
                    <span className="badge success">
                      <Activity size={12} /> Live từ bridge
                    </span>
                  </div>
                  ...
                  <div className="device-card-actions">
                    ...
                    <button
                      ...
                      className="small-button"
                      disabled={isActionBusy}
                      onClick={() => void runDeviceAction(device, "restart")}
                    >
                      {actionId === `restart:${device.id}` ? (
                        <Loader2 size={14} className="spin" />
                      ) : (
                        <RotateCw size={14} />
                      )}{" "}
